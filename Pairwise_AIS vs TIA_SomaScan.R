...
# Pairwise comparison between AIS and TIA (SomaScan Proteomics)
...
#plot(Boruta.IS.TIA.all, las=2, cex.axis=0.7, xlab="", main="Variable Importance")
#plotImpHistory(Boruta.IS.TIA.all)
...
#Sort proteins based on variance explained by Age
head(vp[order(vp$Age, decreasing = TRUE), ])
vpa.age= vp[order(vp$Age, decreasing = TRUE), ]
vpa.age
write.csv(vpa.age, "Protein variance_Age_IS_TIA.csv")

#Sort proteins based on variance explained by Outcome
#head(vp[order(vp$Batch, decreasing = TRUE), ])

#Bar plot of variance fractions for the first 10 genes
plotPercentBars( vp[1:10,]) + ggtitle( "Variance fractions of first 10 proteins between AIS and TIA")
...
#Top proteins identified in at least 2 of the 3 protein selection approaches (AIS vs TIA)
