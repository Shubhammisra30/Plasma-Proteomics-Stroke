...
# Pairwise comparison between ICH and MIM (SomaScan Proteomics)
...
plotPercentBars( vp[1:20,]) + ggtitle( "Variance fractions of first 20 proteins between ICH and MIM")
...
# Top Proteins identified in at least 2 of the 3 protein selection approaches (ICH vs. MIM)

#PCA plot using adjusted top proteins
R.PCA<- prcomp(ICH_MIM_data[[7]][,3:9], scale=TRUE)
R.PCA
X11()
pca_plot= fviz_pca_ind(R.PCA, col.ind=ICH_MIM_data[[7]]$Outcome, title= "PCA plot of adjusted top proteins- ICH vs. MIM", addEllipses = FALSE, label= "none",
                       pointsize= 4)
