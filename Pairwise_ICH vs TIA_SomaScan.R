...
# Pairwise comparison between ICH and TIA (SomaScan Proteomics)
...
#plot(Boruta.ICH.TIA.all, las=2, cex.axis=0.7, xlab="", main="Variable Importance")
#plotImpHistory(Boruta.ICH.TIA.all)
...
plotPercentBars( vp[1:20,]) + ggtitle( "Variance fractions of first 20 proteins between ICH and TIA")
...
# Top Proteins identified in at least 2 of the 3 protein selection approaches (ICH vs. TIA)

#PCA plot using adjusted top proteins
R.PCA<- prcomp(ICH_TIA_data[[7]][,3:6], scale=TRUE)
R.PCA
X11()
pca_plot= fviz_pca_ind(R.PCA, col.ind=ICH_TIA_data[[7]]$Outcome, title= "PCA plot of adjusted top proteins- ICH vs. TIA", addEllipses = FALSE, label= "none",
                       pointsize= 4)
